...
- **Disk sync for saved `.cs`** — after `load_workspace`, one recursive `FileSystemWatcher` per watch root records dirty source paths (not every keystroke: unsaved editor buffers are ignored). Watch roots are the minimal non-nested set of the solution folder plus every loaded project's directory — projects outside the solution folder (e.g. `..\Shared\Lib\Lib.csproj`) are covered too; duplicate/nested directories are deduplicated, and a pathological solution is capped at 16 roots (a warning is logged, the first 16 are watched). Before `find_symbol_*` / `find_usages` / `get_class_skeleton` / test discovery, only those files are read and applied with one `TryApplyChanges`. Host/git/`dotnet format` edits of existing files show up without `reset_workspace`. New `.cs` under a project folder are `AddDocument`’d; deleted files are removed. `.csproj`/`.sln`/`Directory.Build.props` set a graph-stale hint and skip the `load_workspace` cache — still no automatic `OpenSolutionAsync` from the watcher. `reset_workspace` remains for generated `obj` files after build. Linux uses inotify (more roots mean more watches; watch-limit errors log and degrade to a full re-read; they do not crash the process).
